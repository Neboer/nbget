...
#define EXPECT_DOWNLOAD_TIME_SEC 5
...
void
cover_or_trim_task(const proxy proxy_server, list<Task *>::iterator task_i, const string filename,
    ...
    if (proxy_server.speed * EXPECT_DOWNLOAD_TIME_SEC + (*task_i)->start > (*task_i)->end) {
        ...
        *task_i = new_task;
        ...
        // 尝试将这一块失败的下载“切开”
        Task *task1 = new Task(proxy_server, (*task_i)->start,
                               (*task_i)->start + proxy_server.speed * EXPECT_DOWNLOAD_TIME_SEC);
        ...
        if (!taskMessageQueue.empty()) {
            ...
            if (task_reported->status == STATUS_ERROR) {
                ...
                if (!pending_proxy_servers.empty()) {
                    proxy useful_proxy = pending_proxy_servers.front();
